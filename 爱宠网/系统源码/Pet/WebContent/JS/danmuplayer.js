...
(function($) {
	...
	var DanmuPlayer = function(element, options) {
		...
		danmu_video = videojs("#danmu_video", {
			...
			function query() {//1
				$.get(options.url_to_get_danmu, function(data, status) {
					...
					$.each(json,function(index,item){	
						$('#danmu71452').danmu("add_danmu",json[index]);
					});
					});
				}
			...
			$(".vjs-control-bar").append('<input  role="botton" type="textarea" id="danmu_text" name="danmu_text" max=300/>'); // ->
																																// button
...
if(userstr!=null){
	...
	var text_obj1 = JSON.stringify(text_obj);
	//alert(text_obj1);
